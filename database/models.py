...
from sqlalchemy import (
    ...
    Float, ForeignKey, Text, JSON, Index
)
...
class User(Base):
    ...
    # Relationships
    primary_class = relationship('Class', foreign_keys=[class_id], back_populates='members')
    enrolled_classes = relationship('UserClass', back_populates='user', cascade='all, delete-orphan')
    attendance_records = relationship('Attendance', back_populates='user', cascade='all, delete-orphan')
    statistics = relationship('AttendanceStatistics', back_populates='user', cascade='all, delete-orphan')
...
class Class(Base):
    ...
    # Relationships
    members = relationship('User', foreign_keys=[User.class_id], back_populates='primary_class')
    teacher = relationship('User', foreign_keys=[teacher_id])
    leader = relationship('User', foreign_keys=[leader_id])
    enrolled_users = relationship('UserClass', back_populates='class_obj', cascade='all, delete-orphan')
    attendance_records = relationship('Attendance', back_populates='class_obj', cascade='all, delete-orphan')
    statistics = relationship('AttendanceStatistics', back_populates='class_obj', cascade='all, delete-orphan')
...
class AttendanceStatistics(Base):
    ...
    # Composite index
    __table_args__ = (
        Index('idx_stats_user_month', 'user_id', 'month'),
    )
