...
async def add_student_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message += "⚙️ " + (
        get_translation(lang, "feature_coming_soon")
        if lang == "en"
        else "هذه الميزة قادمة قريباً!"
    )
    message += "\n\n"
    message += (
        get_translation(lang, "please_wait")
        if lang == "en"
        else "سيتم إضافتها في المرحلة 5"
    )
...
async def bulk_operations_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message += (
        get_translation(lang, "please_wait")
        if lang == "en"
        else "سيتم إضافتها في المرحلة 7"
    )
