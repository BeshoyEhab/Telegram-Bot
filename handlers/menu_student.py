...
async def view_my_attendance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        await query.edit_message_text(
            get_translation(lang, "user_not_found")
        )
        return
    ...
    message += "📅 " + (get_translation(lang, 'recent_records') if lang == "en" else "السجلات الأخيرة") + ":\n"
    message += "-" * 30 + "\n"
...
def register_student_handlers(application):
    ...
    application.add_handler(CallbackQueryHandler(
        view_my_attendance, 
        pattern="^student_my_attendance$"
    ))
    application.add_handler(CallbackQueryHandler(
        view_my_details, 
        pattern="^student_my_details$"
    ))
    application.add_handler(CallbackQueryHandler(
        view_my_statistics, 
        pattern="^student_my_stats$"
    ))
